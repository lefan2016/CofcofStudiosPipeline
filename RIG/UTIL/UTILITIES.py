...
# @Last modified time: 2017-11-01T00:32:06-03:00
        ...
        for keys, value in archivosCarpetas.items():
                if value:
                    ...
                    if not cmds.objExists(obj+'.'+att+'_VIS') and not cmds.objExists(obj+'.'+att):
                        a=cmds.addAttr(obj, ln=att+"_VIS",keyable=True, at="enum", en="off:on:",dv=1)
                        b=cmds.addAttr(obj, ln=att, keyable=True ,  min=0 , max = rangeVariable  , dv=0 , at='long')
                        c=cmds.addAttr(obj, ln=att+"_ROT", keyable=True ,  min=-180 , max = 180  , dv=0 , at='long')
                        print 'Se creo los atributos en el control ',a,b,c, ' en ',obj
                else:
                    print 'No existe archivos png en ',keys
                    cmds.warning('No existe archivos png en ',keys)
    else:
        print 'No ruta de carpetas.'
...
###############################################################################################################################
=======
# @Date:   2017-10-20T18:45:50-03:00
# @Last modified time: 2017-10-21T01:20:36-03:00
...
def addAttr_FromFolders( sel , path , extension , filtering , contarArchivos): #del archivosCarpetas
    ...
    for key in sorted( archivosCarpetas.keys() ):
        ...
        sel.addAttr( att , keyable=True ,  min=0 , max = frames  , dv=0 , at='short')
